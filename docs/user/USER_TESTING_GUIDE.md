...
## Running Components Directly (Interactive / Login Node)

Use this approach when you want to start each component manually in a separate
terminal rather than through a batch script. You still need a Slurm allocation
so that the containers can access the HSN network; the difference is that you
SSH directly to each node and run the wrapper script there instead of using
`srun`.
